export function renderProgressBar(value: number, max: number): HTMLElement {
  ...
  wrap.innerHTML = `
    <div style="height:10px;border:1px solid var(--border-color);border-radius:999px;overflow:hidden;background-color:var(--surface-alt)">
      <div style="height:100%;width:${(ratio * 100).toFixed(1)}%;background-color:var(--accent)"></div>
    </div>
  `
  return wrap
}
